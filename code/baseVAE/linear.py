...
def Linear(
    ...
    with tf.name_scope(name) as scope:
        ...
        # edited on 2019-03-28
        if initialization == 'w_init': 
            # truncated normal distribution
            def get_truncated_normal(mean, sd):
                return stats.truncnorm(
                    -2, 2, loc=mean, scale=sd)
            
            X = get_truncated_normal(mean=0, sd=0.01)
            weight_values = X.rvs([input_dim, output_dim]).astype('float32')

        elif initialization == 'lecun':# and input_dim != output_dim):
        ...
        # edited on 2019-03-15
        elif initialization == 'he_norm':
